...
parser = OptionParser.new do |cli|
  cli.banner = 'Usage: extract_n4_batch.rb --from N --to M'
  cli.on('--from NUMBER', Integer, 'First N4 source_order (inclusive)') { |value| options[:from] = value }
  cli.on('--to NUMBER', Integer, 'Last N4 source_order (inclusive)') { |value| options[:to] = value }
end
parser.parse!
...
SourceCLI.ensure_exists!(SourceCLI::JMDICT_PATH, SourceCLI::WARODAI_PATH, SourceCLI::N4_PATH)

queue = DictionarySources::N4Queue.new(SourceCLI::N4_PATH)
...
orders.each_with_index do |order, i|
  ...
  SourceCLI.write_json(result, format('tmp/source-extracts/n4-%06d.json', order))
  unresolved << [order, jmdict_matches.length] unless jmdict_matches.length == 1
end

unresolved.each do |order, count|
  warn "N4 source_order #{order} resolved to #{count} JMdict entries; inspect the dossier and reconcile explicitly."
end
